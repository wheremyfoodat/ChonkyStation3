    ...
    if (file_cb_ptr) {
        ...
        while (true) {
            ...
            if (res->result == CELL_SAVEDATA_CBRESULT_OK_LAST || res->result == CELL_SAVEDATA_CBRESULT_OK_LAST_NOCONFIRM) {
                log("Savedata saved successfully\n");
                break;
            }
            
            // Was there an error?
            if (res->result != CELL_SAVEDATA_CBRESULT_OK_NEXT) {
                //Helpers::panic("cellSaveDataUserAutoSave: file cb returned %d\n", (u32)res->result);
                log("cellSaveDataUserAutoSave: file cb returned %d\n", (u32)res->result);
            ...
            if (file_set->file_name) {
                ...
                default: Helpers::panic("cellSaveDataUserAutoSave: unimplemented filetype %d\n", (u32)file_set->filetype);
